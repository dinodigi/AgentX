    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="flex items-center gap-3.5">
        ...
        <div className="min-w-0">
          ...
          <p className="m-0 font-mono text-[11px] text-[--color-line-strong]">prj_{p.projectId.slice(0, 8)}</p>
        </div>
      </div>

      {/* Front door: the two endpoints that define the backend */}
      <section className="rounded-xl border border-[--color-line] bg-[--color-card] p-5">
        <div className="grid gap-4 sm:grid-cols-2">
          <EndpointField label="Delivery API" value={p.deliveryBase} />
          <EndpointField label="MCP endpoint" value={p.mcpEndpoint} />
        </div>
        <p className="mt-3.5 text-[12.5px] leading-relaxed text-[--color-ink-mute]">
          ...
          <Link href={`/admin/${p.projectId}/settings`} className="text-[--color-ink] underline decoration-[--color-line-strong] underline-offset-2 hover:decoration-[--color-ink]">
            Settings
          </Link>
          .
        </p>
      ...
      {/* Operations strip */}
      <section className="flex flex-wrap items-center gap-x-8 gap-y-4 rounded-xl border border-[--color-line] bg-[--color-card] px-5 py-4">
        ...
        <span className="hidden h-9 w-px bg-[--color-line] sm:block" />
      ...
      <div className="grid gap-8 lg:grid-cols-[1.7fr_1fr]">
        <section>
          <div className="mb-3 flex items-baseline justify-between">
            ...
            <span className="font-mono text-[11px] text-[--color-line-strong]">
          ...
          {hasCollections ? (
            <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
              {p.collections.map((c) => (
                <Link
                  ...
                  className="group flex flex-col gap-2 rounded-xl border border-[--color-line] bg-[--color-card] p-4 transition-colors hover:border-[--color-line-strong]"
                >
                  <div className="flex items-center gap-2">
                    ...
                    {c.workflow && <Workflow className="h-3.5 w-3.5 text-[--color-line-strong]" />}
                    {c.unhandled > 0 && (
                      <span
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-mono text-[10px] text-[--color-line-strong]">{ago(c.lastActivity)}</span>
                  </div>
                </Link>
              ))}
            </div>
            ...
            <div className="card flex flex-col items-start gap-2 p-8">
              ...
              <Link
                ...
                className="mt-1 inline-flex items-center gap-1 font-mono text-[12px] text-[--color-ink] hover:text-[--color-accent]"
              >
        ...
        <section>
          ...
          {p.activity.length > 0 ? (
            ...
            <div className="rounded-xl border border-[--color-line] bg-[--color-card] p-6 text-center">
              <p className="font-mono text-[11px] text-[--color-ink-mute]">no activity yet</p>
            </div>
          )}
        </section>
      </div>
