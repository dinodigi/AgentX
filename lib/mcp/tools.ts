...
export const TOOL_DEFS: ToolDef[] = [
  ...
  },
  {
    ...
    description:
      "Create or update (upsert by name) a recurring schedule that fires a webhook, an email, or a " +
      "DECLARATIVE DATA MUTATION — presets only, no cron strings. recurrence: {frequency: " +
      "hourly|daily|weekly|monthly, at: 'HH:MM' (24h UTC; not for hourly), weekday (weekly), " +
      "dayOfMonth 1-28 (monthly)}. UTC-only for now. action: {type:'webhook',url} | " +
      "{type:'email',to,subject,html?} | {type:'mutate', collection, where:[{field,op,value}], " +
      "guard?:[...], transition?:{to}, set?:{field: spec}} — the sweep that used to need external " +
      "compute. MUTATE semantics (closed vocabulary, no arithmetic/branching): `where` selects rows " +
      "(≥1 clause; value may be a RELATIVE time {daysAgo:n}|{hoursAgo:n}, resolved at each tick); " +
      "`guard` re-checks atomically at write time (CAS — changed rows are skipped, never stomped; " +
      "defaults to where); `transition.to` moves the collection's workflow (validated as the mcp " +
      "actor); `set` stamps fields: \"now\" | null (unset) | {value:<literal>} | {copyFrom:'<field>'} " +
      "(e.g. previous_owner={copyFrom:'owner'}). Bounded to 200 rows per tick — the next tick " +
      "continues. Audit rows carry the schedule name. Fires are at-least-once and idempotent " +
      "(already-at-target rows skip); a missed window fires once, never backfills. enabled:false " +
      "pauses AND skips queued fires. Webhook/email outcomes land in get_deliveries; pending fires " +
      "in list_jobs.",
    inputSchema: {
      type: "object",
      properties: {
        ...
        action: {
          type: "object",
          description:
            "{type:'webhook',url} | {type:'email',to,subject,html?} | {type:'mutate', collection, " +
            "where:[{field,op,value|{daysAgo}|{hoursAgo}}], guard?, transition?:{to}, " +
            "set?:{field: \"now\"|null|{value}|{copyFrom}}}",
        },
