...
| DX-6 | **MCP OAuth — connect with a URL, nothing else** (operator, 2026-07-22; "the one I like most"). The MCP spec supports authorization for remote servers: a client is given `https://pluggie.app/api/mcp`, completes auth in a browser, and is connected — no token pasting, no config file, no CLI. Today the ONLY way to get a token is a human clicking in the console, which is why setting up MCP feels fuzzy on every new project. **Depends on TOK-1 (programmatic issuance) on top of MT-1 (bounded tokens)** — MT-1 → TOK-1 → DX-6 is one capability in three layers, not three items. Interim fallback if OAuth proves slow: a `npx pluggie init` CLI that authenticates, mints, and writes `.mcp.json` (~1 day, same dependency). | 📥 Backlog | **H** | operator |
