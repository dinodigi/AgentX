...
| TOK-1 | **Mint/rotate delivery tokens over MCP** — `mint_delivery_token` / `list_delivery_tokens` / `revoke_delivery_token`, delivery-scope hard-fixed, parentage + DB-level cascade revoke (`minted_by_token_id`), cap 25/project, platform-events trail, console "agent" chip. Success criterion "empty project → working delivery calls without leaving MCP" met. Unblocks DX-6 (MCP OAuth). | ✅ Shipped (sprint 2026-07, unpushed) | **H** | wall ×2 |
| TOK-2 | **Sub-issue kept from TOK-1:** a mint that would exceed the cap answers `E_CAP_REACHED` with the remedy — consider surfacing cap headroom in `get_project_info.briefing.health` so agents see it BEFORE minting. | 🅿️ Parked | L | design |
| PLUG-3 | **`enabled` ≠ `applied`** — shipped as EVIDENCE-not-verdict: `applied` `{status: none\|unclear\|full, matched, of, unmatched, nextAction}`, fresh-read, ends-only assertions ("partial" was provably wrong on CSLP — reconciled renames are CORRECT behavior). | ✅ Shipped (sprint 2026-07, unpushed) | M | wall (Codex) |
| PLUG-4 | **Kept from PLUG-3:** the durable fix — stamp REALIZED collection names on `project_plugins` at apply time, turning applied-state from a name-match guess into a fact (also kills the renamed-everything false-`none`). Wants a hand-applied migration; batch with the next schema change. | 📥 Backlog | M | design |
| OPS-3 | **Health check: split liveness from readiness** — shipped: 200 + degraded body, keyword-monitor requirement documented, degraded path verified by simulation (`scripts/verify-health-degraded.mjs`). ~~Cost claim~~ retracted in the sprint plan (the every-minute drain cron keeps the DB awake regardless). | ✅ Shipped (sprint 2026-07, unpushed) | **H** | incident |
...
| PLUG-2 | **Base/blueprint composition model** — now a full plan: [plans/PLUGIN-BASES-PLAN.md](plans/PLUGIN-BASES-PLAN.md) (Phase 1 = composition core for bases + AUTO-1 + drift/session briefing + wave-1 bases + operator poke project; Phase 2 blueprints DEFERRED until bases mature). Ideation catalog: [PLUGIN-BASE-CATALOG.md](PLUGIN-BASE-CATALOG.md). | 🚧 In progress | H | design, wall |
| PANEL-1 | **In-chat panels via MCP Apps** (the official MCP UI extension) — tools that return interactive panels rendered in the builder's chat: test console driven by plugin `acceptance` arrays, onboarding wizard, session-briefing dashboard; potentially a FOURTH plugin ingredient (structure/guidance/verbs/**panel** — e.g. `workboard` ships a kanban face). Host-support gated (rich hosts render, terminals degrade to console links). Explore after PLUG-2 lands. | 🅿️ Parked | M | design |
...
| CONN-2 | **SMS connector** (Twilio-shaped) — `{type:'sms', to:'{{phone}}'}` event actions gated on consent flags; the countryside baseline ships `text_opt_in` with nothing to act on. *(Unblocked: CONN-1's registry shipped — this is now a new `sms` category + adapter, following the email pattern.)* | 📥 Backlog | M | wall |
