...
| WP-9 | **`publicRead` naming trap on gated collections** (wall `21f4c5d5`, xvibe): `publicRead` gates FIELD visibility for *every* delivery reader, including authenticated ones — the name reads as "anonymous-only", so an agent setting `access.read: authenticated` reasonably skipped `publicRead` on staff fields and got rows of just `id`. Fix: define-time `constraintWarnings[]` when a gated collection has zero/near-zero delivery-visible fields, + contract copy stating "publicRead = delivery-visible, not anonymous-only" (CONTRACT-1 carries the copy half). | 📥 Backlog | M | wall (xvibe) |
| WF-1 | **Stamp fields on workflow transitions** (wall `0cd6dce5`, xvibe): `transitions[].set` reusing AUTO-1's closed `set` vocabulary (`"now"` / `{value}` / `null`) so `resolved_at` stamps AT transition time — today transition actions fire webhooks/emails but cannot mutate the entry, and scheduled mutations only stamp on sweep cadence. Same write path as the transition (one statement), same define-time validation as schedule `set` (including the SEC-1 copyFrom/write-only gates if copyFrom is ever allowed here). | 📥 Backlog | M | wall (xvibe) |
...
| DX-7 | **`dryRun` on `define_collection`** (wall `61f9b82e`, xvibe): validate + return the FULL diff plan, apply nothing — today only destructive changes preview (confirm plans), and `transact` already has the precedent. Turns agent plan-mode schema steps into reviewable diffs; pairs with QRY-4's environments story. | 📥 Backlog | M | wall (xvibe) |
| OPS-6 | **One-call `reset_project`** (wall `ad7568ba`, xvibe): wiping a burn/test project takes N `delete_collection` calls in dependency order (E_BLOCKED on relation targets) plus schedule/token/plugin cleanup. Confirm-gated (plan first, like delete_collection), `schema.manage`-scoped; our own smoke harness wants it too (the stranded-project sweeper exists because teardown is hard). | 📥 Backlog | M | wall (xvibe) |
...
| ID | Item | Status | Pri | Source |
|---|---|---|---|---|
| CONTRACT-1 | **Full pass over agent-facing language** — every tool description, `get_project_info`, `list_field_types`, error copy, and the generated client. Accurate, complete, discoverable, self-correcting, self-contained. Umbrella over WP-6-docs, DX-1, DX-2. *(CP10 knocked three defects off its checklist: WP-3 contradiction fixed, WP-4 semantics stated, QRY-3 budgets published — each pinned by a contract test, which is the anti-regression method the detail section prescribes.)* **This is the next flagship initiative now the wall is clear — and its FIRST deliverable is decided: the machine-readable not-supported registry in the session briefing** (wall `2479b787`, xvibe — self-proving: two of the reporter's four "unsupported" examples had already shipped, which is what a hand-maintained list in a client prompt does). (See detail.) | 📥 Backlog | H | design, dogfood, audit, wall (xvibe) |
