...
| PLUG-2 | **Base/blueprint composition model** — bases own ONE capability (`provides`), declare deps (`requires`); blueprints bundle bases (`includes`); enable rule = one active provider per capability (replaces overlap hints with enforcement). Full base ideation: [PLUGIN-BASE-CATALOG.md](PLUGIN-BASE-CATALOG.md) (~40 bases, 6 parked on named primitives). Countryside splits into the first blueprint as the proving refactor. | 🗓️ Phased | H | design, wall |
