...
| TOK-1 | **Mint/rotate delivery tokens over MCP** — **TWO independent reports.** Stallion: an agent can't self-heal a broken deployment without dashboard access. Codex (poke run): `get_client_code` GENERATES a frontend client that requires a delivery token, and no MCP tool can produce one — **the platform hands out code it won't let you run**, so the build loop can't close in one workflow. Privilege-DOWNWARD (MCP is already the master credential; delivery is strictly weaker), so safe in principle; audit-stamp mints. | 📥 Backlog | **H** | wall ×2 |
| PLUG-3 | **`enabled` ≠ `applied`** (Codex poke run) — `list_plugins` shows a plugin enabled the moment it's licensed, so an agent reasonably reads that as "structures are in place" and is surprised by an empty `list_collections`. Add an applied-state (`none`/`partial`/`full`, computed by checking baseline collection names against the project) + an explicit `nextAction`. Also closes a Track C hole: we stamp the enabled VERSION but never whether the structure landed, so a never-applied plugin is indistinguishable from a fully-applied one in the briefing. | 📥 Backlog | M | wall (Codex) |
| OPS-3 | **Health check: split liveness from readiness** — `/api/health` returns 503 when the control DB is unreachable and `render.yaml` points `healthCheckPath` at it, so Render restart-loops every instance and a DB outage becomes a TOTAL blackout (proven 2026-07-21: every route 502, static marketing pages included). Fix: return **200** with `{status:"degraded",db:"down"}` so the instance stays in rotation — static serves, APIs fail honestly per-request, drain cron stays reachable; UptimeRobot still alerts (its keyword `"ok"` is absent). **Also the main cost driver**: constant health polling keeps the control DB awake 24/7 (~180 CU-h/mo ≈ the whole Neon bill). Minutes of work. | 📥 Backlog | **H** | incident |
| OPS-4 | **Dedicated test database** — the smoke suite runs against the production control DB; a ~300-test session exhausted its Neon compute quota and took prod down (2026-07-21). On the paid plan it now costs real money instead of hitting a ceiling. Want a separate Neon project for `SMOKE_BASE` runs. | 📥 Backlog | **H** | incident |
