...
| SEC-4 | **Encrypt `writeOnly` field values at rest.** SEC-1 is redaction + storage minimisation, NOT encryption: the value lands in `entries.data` (JSONB) in PLAINTEXT and is merely never returned by any read path (`grep -rn "encrypt" lib/entries.ts` = 0). So the guarantee holds against every API surface and the admin UI, and NOT against anyone holding the database connection string. We already have the mechanism — `lib/crypto.ts` is AES-256-GCM with a keyring and versioned blobs, used today for connector `secretsEnc`. Reuse `encryptSecret`/`decryptSecret` at the write boundary and the before-write-hook envelope (a hook legitimately receives the value it is asked to validate). Makes "the platform stores credentials safely" an unqualified claim instead of a qualified one. Watch: the field must stay unfilterable/unsortable regardless (a ciphertext comparison is not a value comparison), and a flipped-to-writeOnly field leaves plaintext in history that layer 2 already covers on read but that this would not retroactively encrypt — state that honestly. | 📥 Backlog | M | design (2026-07-31 session) |
