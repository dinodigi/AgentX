...
| Item | Why it's yours |
|---|---|
| `0ceec805` D1 — `auth_kit` leaves credentials to every tenant | **Strategic, not technical.** Store credential material (breaking "credential-free by design"), ship a verified reference implementation, or stay out and document the trap loudly? All three are defensible. The current position is the only one that is *silently* dangerous — the reporter had to know to use a real dummy hash so response latency doesn't enumerate accounts, and said they'd expect most integrators to miss it. |
