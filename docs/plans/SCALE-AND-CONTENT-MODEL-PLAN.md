...
**Status:** A1 + A2 SHIPPED & verified (6221c26, 374642f). **A2 confirmed via EXPLAIN on 20k rows** (filter → Bitmap Index Scan; sort+LIMIT → Index Scan). **A1 fixed**: default pagination was ordering by `date_trunc('ms', created_at)` — a non-immutable expr the raw index can't serve — so A1 was a no-op; now orders by raw `created_at` (index-backed seek) with a **microsecond keyset cursor** (no dup/skip). **Phase B SHIPPED** (2b58762): one level of repeating (`MAX_ARRAY_GROUP_DEPTH: 2→1`) — deeper/repeating lists go to a related collection. **A3 reclassified to Phase C**: A2 already index-backs custom sorts (offset skips *along the index*), so keyset-on-custom-sort only helps at extreme offsets and is null/type/direction-edge-heavy — do it only if deep-sorted pagination becomes a real need. Phase C still to do.
