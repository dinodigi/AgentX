...
**Status:** COMPLETE — Layers 1–3b SHIPPED & verified (2026-07-15, local master, not pushed). L1 group/array types + recursive validation + caps (76f44a1). L2 recursive projection (F3) + write-gate (F2) + nested asset resolution (7de4788). L3a admin JSON editor (0a5c7f1). L3b visual repeater editor `StructuredFieldEditor` (9344011, browser-verified). Structured content works across MCP, the admin (visual add/remove/nested), and delivery (safe). Only reorder-drag is deferred polish. **UPDATE (Scale Phase B): repeater-in-repeater was walked back to ONE level of repeating (`MAX_ARRAY_GROUP_DEPTH: 2→1`) — a deeper/repeating list is modeled as a related collection (queryable/searchable/reusable). Scalar sub-arrays like tags inside a repeater item are still allowed. Existing depth-2 collections are grandfathered (reads/writes work; can't be redefined to depth-2).**
